...
func _ready():
    ...
    AudioManager.BGM_mute_state_changed.connect(func(_is_muted: bool):
        mute_bgm_button.set_pressed_no_signal(_is_muted)
    )

    AudioManager.SE_mute_state_changed.connect(func(_is_muted: bool):
        mute_se_button.set_pressed_no_signal(_is_muted)
    )
