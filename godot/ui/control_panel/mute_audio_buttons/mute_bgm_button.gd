...
func _ready():
    ...
    var is_muted = AudioManager.is_bgm_muted()
    refresh_light(is_muted)

    AudioManager.BGM_mute_state_changed.connect(
        func(_is_muted: bool):
...
func _on_button_pressed() -> void:
    # toggle SE
    var new_mute_state: bool = not AudioManager.is_bgm_muted()
    AudioManager.mute_bgm(new_mute_state)
